...
import { Share2, User, Mail, MessageSquare, Send, Check } from "lucide-react";
...
const ContactPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Swal.fire({ title: "Error!", text: "Something went wrong. Please try again later.", icon: "error", confirmButtonColor: "#6366f1" });
    } finally {
        ...
        <div className="container px-[1%] grid grid-cols-1 sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-[45%_55%] 2xl:grid-cols-[35%_65%] gap-12">
          <div data-aos="fade-right" data-aos-duration="1200" className="bg-white/5 backdrop-blur-xl rounded-3xl shadow-2xl p-5 py-10 sm:p-10 transform transition-all duration-300 hover:shadow-[#6366f1]/10">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button data-aos="fade-up" data-aos-delay="400" type="submit" disabled={isSubmitting || isSent} className={`w-full py-4 rounded-xl font-semibold transition-all duration-300 flex items-center justify-center gap-2 ${isSent ? 'bg-green-500 text-white shadow-lg' : isSubmitting ? 'bg-gradient-to-r from-[#6366f1] to-[#a855f7] text-white animate-pulse' : 'bg-gradient-to-r from-[#6366f1] to-[#a855f7] text-white hover:scale-[1.02] hover:shadow-lg hover:shadow-[#6366f1]/20 active:scale-[0.98]'}`}>
                {isSent ? (
                  <>
